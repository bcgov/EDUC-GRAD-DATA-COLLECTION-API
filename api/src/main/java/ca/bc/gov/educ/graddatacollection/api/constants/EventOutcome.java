package ca.bc.gov.educ.graddatacollection.api.constants;

/**
 * The enum Event outcome.
 */
public enum EventOutcome {
  INITIATE_SUCCESS,
  SAGA_COMPLETED,
  READ_DEM_STUDENTS_FOR_PROCESSING_SUCCESS,
  READ_ASSESSMENT_STUDENTS_FOR_PROCESSING_SUCCESS,
  READ_COURSE_STUDENTS_FOR_PROCESSING_SUCCESS,
  VALIDATE_DEM_STUDENT_SUCCESS_WITH_NO_ERROR,
  VALIDATE_DEM_STUDENT_SUCCESS_WITH_CRITICAL_ERROR,
  VALIDATE_DEM_STUDENT_SUCCESS_WITH_NON_CRITICAL_ERROR,
  STUDENT_ADDRESS_UPDATED,
  STUDENT_ADDRESS_UPDATE_COMPLETE,
  NO_STUDENT_ADDRESS_UPDATE_REQUIRED,
  DEM_STUDENT_CREATED_IN_GRAD,
  DEM_STUDENT_SOR_UPDATED_IN_GRAD,
  VALIDATE_COURSE_STUDENT_SUCCESS_WITH_NO_ERROR,
  VALIDATE_COURSE_STUDENT_SUCCESS_WITH_ERROR,
  VALIDATE_COURSE_STUDENT_SUCCESS,
  COURSE_STUDENT_CREATED_IN_GRAD,
  COURSE_STUDENT_STATUS_IN_COLLECTION_UPDATED,
  VALIDATE_ASSESSMENT_STUDENT_SUCCESS_WITH_NO_ERROR,
  VALIDATE_ASSESSMENT_STUDENT_SUCCESS_WITH_ERROR,
  VALIDATE_ASSESSMENT_STUDENT_SUCCESS,
  ASSESSMENT_STUDENT_REGISTRATION_PROCESSED,
  ASSESSMENT_STUDENT_NOT_WRITTEN_DUE_TO_DEM_FILE_ERROR,
  ASSESSMENT_STUDENT_REGISTRATION_ALREADY_EXISTS_IN_EAS,
  ASSESSMENT_STUDENT_STATUS_UPDATED,
  DEM_STUDENT_STATUS_IN_COLLECTION_UPDATED,
  STUDENT_REGISTRATION_PROCESSED_IN_ASSESSMENT_API,
  STUDENT_NOT_FOUND,
  GDC_CACHE_UPDATED,
  GRAD_SCHOOL_UPDATED,
  COURSE_STUDENT_CREATED_NOT_WRITTEN_DUE_TO_DEM_FILE_ERROR,
  DEM_STUDENT_PROCESSED_IN_GRAD_STUDENT_API,
  COURSE_STUDENT_PROCESSED_IN_GRAD_STUDENT_API,
  READ_COURSE_STUDENTS_FOR_DOWNSTREAM_UPDATE_SUCCESS,
  COURSE_STUDENT_STATUS_UPDATED,
  READ_COMPLETED_FILESETS_FOR_PROCESSING_SUCCESS,
  COMPLETED_FILESET_STATUS_UPDATED,
  COMPLETED_FILESET_STATUS_AND_SOURCE_SYSTEM_VENDOR_CODE_UPDATED,
  COPY_FILESET_FROM_STAGING_TO_FINAL_TABLE_COMPLETE,
  DELETE_FILESET_FROM_STAGING_COMPLETE,
  COMPLETED_FILESET_STATUS_UPDATED_SOURCE_SYSTEM_VENDOR_CODE_DOES_NOT_NEED_UPDATE,
  SCHOOL_UPDATED,
  
}
