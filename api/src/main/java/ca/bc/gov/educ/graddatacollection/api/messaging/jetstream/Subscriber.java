...
import ca.bc.gov.educ.graddatacollection.api.service.v1.events.EventHandlerDelegatorService;
import ca.bc.gov.educ.graddatacollection.api.service.v1.events.JetStreamEventHandlerService;
...
public class Subscriber {
    ...
    private void initializeStreamTopicMap() {
        ...
        final List<String> gdcEventsTopics = new ArrayList<>();
        gdcEventsTopics.add("GDC_EVENTS_TOPIC");
        this.streamTopicsMap.put("GRAD_SCHOOL_EVENTS", gradSchoolEventsTopics);
        this.streamTopicsMap.put("GDC_EVENTS_TOPIC", gdcEventsTopics);
    }
    ...
    public void subscribe() throws IOException, JetStreamApiException {
        val qName = ApplicationProperties.GRAD_DATA_COLLECTION_API.concat("-QUEUE");
        val autoAck = false;
        for (val entry : this.streamTopicsMap.entrySet()) {
            for (val topic : entry.getValue()) {
                final PushSubscribeOptions options = PushSubscribeOptions.builder().stream(entry.getKey())
                        .durable(ApplicationProperties.GRAD_DATA_COLLECTION_API.concat("-DURABLE"))
