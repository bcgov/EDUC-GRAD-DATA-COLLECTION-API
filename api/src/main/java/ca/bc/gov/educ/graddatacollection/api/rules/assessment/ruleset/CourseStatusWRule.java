...
@Slf4j
@Order(210)
public class CourseStatusWRule implements AssessmentValidationBaseRule {

    private final AssessmentRulesService assessmentRulesService;

    public CourseStatusWRule(AssessmentRulesService assessmentRulesService) {
        this.assessmentRulesService = assessmentRulesService;
    }

    @Override
    public boolean shouldExecute(StudentRuleData studentRuleData, List<AssessmentStudentValidationIssue> validationErrorsMap) {
        log.debug("In shouldExecute of V21: for assessment {} and assessmentStudentID :: {}", studentRuleData.getAssessmentStudentEntity().getAssessmentID() ,
                studentRuleData.getAssessmentStudentEntity().getAssessmentStudentID());

        var shouldExecute = isValidationDependencyResolved("V21", validationErrorsMap);

        log.debug("In shouldExecute of V21: Condition returned - {} for assessmentStudentID :: {}" ,
                shouldExecute,
                studentRuleData.getAssessmentStudentEntity().getAssessmentStudentID());
    ...
    public List<AssessmentStudentValidationIssue> executeValidation(StudentRuleData studentRuleData) {
        ...
        log.debug("In executeValidation of V21 for assessmentStudentID :: {}", student.getAssessmentStudentID());
        ...
        var currentReg = studentRuleData.getAssessmentStudentDetail() != null && studentRuleData.getAssessmentStudentDetail().isHasPriorRegistration();

        if (!currentReg && studentRuleData.getAssessmentStudentEntity().getCourseStatus().equalsIgnoreCase("W")){
            log.debug("V21: Error The registration does not exist in the student record so it cannot be withdrawn. :: {}", student.getAssessmentStudentID());
            errors.add(createValidationIssue(StudentValidationIssueSeverityCode.ERROR, ValidationFieldCode.COURSE_STATUS, AssessmentStudentValidationIssueTypeCode.COURSE_STATUS_W_INVALID, AssessmentStudentValidationIssueTypeCode.COURSE_STATUS_W_INVALID.getMessage()));
        }
        return errors;
    }
