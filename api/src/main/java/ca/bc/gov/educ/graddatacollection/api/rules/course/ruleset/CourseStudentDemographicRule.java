...
@Slf4j
@Order(20)
public class CourseStudentDemographicRule implements CourseValidationBaseRule {

    private final CourseRulesService courseRulesService;

    public CourseStudentDemographicRule(CourseRulesService courseRulesService) {
        this.courseRulesService = courseRulesService;
    }

    @Override
    public boolean shouldExecute(StudentRuleData studentRuleData, List<CourseStudentValidationIssue> validationErrorsMap) {
        log.debug("In shouldExecute of C02: for course {} and courseStudentID :: {}", studentRuleData.getCourseStudentEntity().getCourseStudentID() ,
                studentRuleData.getCourseStudentEntity().getCourseStudentID());

        var shouldExecute = isValidationDependencyResolved("C02", validationErrorsMap);

        log.debug("In shouldExecute of C02: Condition returned - {} for courseStudentID :: {}" ,
                shouldExecute,
                studentRuleData.getCourseStudentEntity().getCourseStudentID());
    ...
    public List<CourseStudentValidationIssue> executeValidation(StudentRuleData studentRuleData) {
        ...
        log.debug("In executeValidation of C02 for courseStudentID :: {}", student.getCourseStudentID());
        ...
        if (!RuleUtil.validateStudentRecordExists(studentRuleData.getStudentApiStudent())){
            log.debug("C02: Student CRS record will not be processed due to an issue with the student's demographics PEN :: {}", student.getCourseStudentID());
            ...
            if (!RuleUtil.validateStudentSurnameMatches(demographicStudentEntity, studentRuleData.getStudentApiStudent())) {
                log.debug("C02: Student CRS record will not be processed due to an issue with the student's demographics LAST NAME :: {}", student.getCourseStudentID());
            ...
            if (!RuleUtil.validateStudentGivenNameMatches(demographicStudentEntity, studentRuleData.getStudentApiStudent())) {
                log.debug("C02: Student CRS record will not be processed due to an issue with the student's demographics MIDDLE NAME :: {}", student.getCourseStudentID());
            ...
            if (!RuleUtil.validateStudentMiddleNameMatches(demographicStudentEntity, studentRuleData.getStudentApiStudent())) {
                log.debug("C02: Student CRS record will not be processed due to an issue with the student's demographics FIRST NAME :: {}", student.getCourseStudentID());
            ...
            if (!RuleUtil.validateStudentDOBMatches(demographicStudentEntity, studentRuleData.getStudentApiStudent())) {
                log.debug("C02: Student CRS record will not be processed due to an issue with the student's demographics DOB :: {}", student.getCourseStudentID());
