...
@Service
@Slf4j
public class ProcessAssessmentStudentsEventDelegatorService implements EventHandler {
  /**
   ...
   */
  @Autowired
  public ProcessAssessmentStudentsEventDelegatorService(final EventHandlerService eventHandlerService, final EventPublisherService eventPublisherService) {
  ...
  public void handleEvent(final Event event) {
    try {
      if(event.getEventType() == EventType.READ_ASSESSMENT_STUDENTS_FOR_PROCESSING) {
        log.debug("Received read from topic event :: ");
        log.trace(PAYLOAD_LOG, event.getEventPayload());
        this.getEventHandlerService().handleProcessAssessmentStudentsEvent(event); // no response in this event.
      }
      else {
  ...
  public String getTopicToSubscribe() {
    return TopicsEnum.READ_ASSESSMENT_STUDENTS_FROM_TOPIC.toString();
  }
