...
public class ProcessCompletedFilesetsEventDelegatorService implements EventHandler {
    ...
    public void handleEvent(final Event event) {
        try {
            if(event.getEventType() == EventType.READ_COMPLETED_FILESETS_FOR_PROCESSING) {
                ...
                this.getEventHandlerService().handleProcessIncomingFilesetsEvent(event); // no response in this event.
            }
            else {
    ...
    public String getTopicToSubscribe() {
        return TopicsEnum.READ_COMPLETED_FILESETS_FROM_TOPIC.toString();
    }
