...
@Service
@Slf4j
public class ProcessCourseStudentsForDownstreamUpdateEventDelegatorService implements EventHandler {
  /**
   ...
   */
  @Autowired
  public ProcessCourseStudentsForDownstreamUpdateEventDelegatorService(final EventHandlerService eventHandlerService, final EventPublisherService eventPublisherService) {
  ...
  public void handleEvent(final Event event) {
    try {
      if(event.getEventType() == EventType.READ_COURSE_STUDENTS_FOR_DOWNSTREAM_UPDATE) {
        log.debug("Received read from topic event :: ");
        log.trace(PAYLOAD_LOG, event.getEventPayload());
        this.getEventHandlerService().handleProcessCourseStudentsForDownstreamUpdateEvent(event); // no response in this event.
      }
      else {
  ...
  public String getTopicToSubscribe() {
    return TopicsEnum.READ_COURSE_STUDENTS_FOR_DOWNSTREAM_UPDATE_TOPIC.toString();
  }
