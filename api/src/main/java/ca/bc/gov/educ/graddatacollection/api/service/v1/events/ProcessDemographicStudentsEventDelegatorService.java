...
@Service
@Slf4j
public class ProcessDemographicStudentsEventDelegatorService implements EventHandler {
  /**
   ...
   */
  @Autowired
  public ProcessDemographicStudentsEventDelegatorService(final EventHandlerService eventHandlerService, final EventPublisherService eventPublisherService) {
  ...
  public void handleEvent(final Event event) {
    try {
      if(event.getEventType() == EventType.READ_DEM_STUDENTS_FOR_PROCESSING) {
        log.debug("Received read from topic event :: ");
        log.trace(PAYLOAD_LOG, event.getEventPayload());
        this.getEventHandlerService().handleProcessDemStudentsEvent(event); // no response in this event.
      }
      else {
  ...
  public String getTopicToSubscribe() {
    return TopicsEnum.READ_DEM_STUDENTS_FROM_TOPIC.toString();
  }
